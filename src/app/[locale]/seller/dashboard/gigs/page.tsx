...
export default function SellerGigsPage() {
  ...
  const handlePauseGig = async (gigId: string) => {
    if (!user) return;
    try {
      await pauseGig(gigId, user.id);
      showToast.success('Gig paused');
      loadGigs();
    } catch (error) {
      console.error('Error pausing gig:', error);
      showToast.error('Failed to pause gig');
    }
  };
  ...
  const handleActivateGig = async (gigId: string) => {
    if (!user) return;
    try {
      await activateGig(gigId, user.id);
      showToast.success('Gig activated');
      loadGigs();
    } catch (error) {
      console.error('Error activating gig:', error);
      showToast.error('Failed to activate gig');
    }
  };
