...
export default function EditGigPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        imageUrls: [] as string[],
        packages: {
            basic: { name: '', description: '', price: '', deliveryDays: '', revisions: '', features: [''] },
            standard: { name: '', description: '', price: '', deliveryDays: '', revisions: '', features: [''] },
            premium: { name: '', description: '', price: '', deliveryDays: '', revisions: '', features: [''] },
        },
    ...
    useEffect(() => {
        ...
        if (!user) {
            router.push(`/${locale}/login?redirect=/${locale}/seller/gigs/${gigId}/edit`);
            return;
        }
    ...
    }, [user, isLoading, router, locale, gigId]);
            ...
            try {
                ...
                if (gig.packages) {
                    gig.packages.forEach((pkg: any) => {
                        const tier = pkg.tier as keyof typeof packagesObj;
                        packagesObj[tier] = {
                            name: pkg.name || '',
                            description: pkg.description || '',
                            price: pkg.price ? pkg.price.toString() : '',
                            deliveryDays: pkg.delivery_days ? pkg.delivery_days.toString() : '',
                            revisions: pkg.revisions === null ? 'unlimited' : pkg.revisions.toString(),
                ...
                // Parse requirements
                const reqs = (gig.requirements || []).map((req: any) => ({
                    question: req.question || '',
                    answerType: req.answer_type || 'text',
                ...
                setFormData(prev => ({
                    ...prev,
                    title: gig.title || '',
                    category: gig.category_id || gig.category || '',
                    subcategory: gig.subcategory_id || gig.subcategory || '',
                    tags: gig.tags || [],
                    deliveryType: gig.delivery_type || 'service',
                ...
                console.error(err);
                showToast.error('Failed to load gig data');
            } finally {
                setLoadingGig(false);
            }
        };
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            const remainingSlots = 5 - formData.images.length - formData.imageUrls.length;
    ...
    const handlePublish = async () => {
        ...
        try {
            // Upload new images
            const uploadedImageUrls = [...formData.imageUrls];
            ...
            if (!gigResponse.ok) throw new Error('Failed to update gig');
            ...
            const packagesToCreate = ['basic', 'standard', 'premium']
                ...
                .map((tier) => {
                    ...
                    return {
                        tier,
                        name: pkg.name || `${tier.charAt(0).toUpperCase() + tier.slice(1)} Package`,
                        description: pkg.description,
            ...
            const packagesResponse = await fetch(`/api/gigs/${gigId}/packages`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userId: user.id,
            ...
            if (!packagesResponse.ok) throw new Error('Failed to update packages');

            // Create/Update requirements
            const reqsResponse = await fetch(`/api/gigs/${gigId}/requirements`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userId: user.id,
            ...
            if (!reqsResponse.ok) throw new Error('Failed to update requirements');
            ...
            showToast.error(err.message || 'Failed to update gig');
        } finally {
            setLoading(false);
        }
    };
                        ...
                        {STEPS.map((step, index) => (
                            <div key={step.id} className="flex-1 relative">
                                <div className="flex items-center">
                                    <div
                                        className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center border-2 ${currentStep > step.id
                                ...
                                {index < STEPS.length - 1 && (
                                    <div
                                        className={`absolute top-5 left-10 right-0 h-0.5 ${currentStep > step.id ? 'bg-brand-green' : 'bg-gray-300'
                                            }`}
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="I will..."
                                    maxLength={80}
                                    className={`w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-green ${errors.title ? 'border-red-500' : 'border-gray-300'
                                        }`}
                                />
                                {errors.title && (
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className={`w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-green ${errors.category ? 'border-red-500' : 'border-gray-300'
                                            }`}
                                    >
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                placeholder="1000"
                                                min="100"
                                                className={`w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-green ${errors[`${tier}.price`] ? 'border-red-500' : 'border-gray-300'
                                                    }`}
                                            />
                                            {errors[`${tier}.price`] && (
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                placeholder="3"
                                                min="1"
                                                className={`w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-green ${errors[`${tier}.deliveryDays`] ? 'border-red-500' : 'border-gray-300'
                                                    }`}
                                            />
                                        </div>
                                    </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className={`w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-green ${errors.description ? 'border-red-500' : 'border-gray-300'
                                        }`}
                                />
                                {errors.description && (
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-4 gap-4 mt-4">
                                    {formData.imageUrls.map((url, index) => (
                                        <div key={`url-${index}`} className="relative w-full h-24">
                                            <Image
                                                src={url}
                                    ...
                                    {formData.images.map((file, index) => (
                                        <div key={`file-${index}`} className="relative w-full h-24">
                                            <Image
                                                src={URL.createObjectURL(file)}
                                                alt={`Preview File ${index + 1}`}
                                                fill
                                                className="object-cover rounded-lg"
                                            />
                                            <button
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-gray-50 rounded-lg p-6 space-y-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-700">{categories.find(c => c.id === formData.category)?.name || formData.category}</p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-700">{formData.imageUrls.length + formData.images.length} image(s) attached</p>
                                </div>
