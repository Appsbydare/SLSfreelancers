...
export default function BecomeTaskerPage() {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (googleError) {
        if (googleError === 'google_not_configured') {
          alert('Google Sign In is not configured yet. Please use the form below instead.');
        } else if (googleError === 'oauth_cancelled') {
          // User cancelled, no need to show error
        } else {
          alert('Google sign in failed. Please try again or use the form below.');
        }
        // Clear URL params
        ...
        setQuickSignupData(prev => ({
          ...prev,
          callingName: googleFirstName || fullName || '',
        }));
        ...
        // If profile picture URL is available, we could use it
        // For now, just clear URL params
        window.history.replaceState({}, '', window.location.pathname);
      }
    }
  }, []);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // Become a Tasker page - use tasker userType
      const googleAuthUrl = `/api/auth/google?userType=tasker`;
