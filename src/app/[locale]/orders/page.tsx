...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      setLoading(true);
      try {
        const userType = user.userType === 'tasker' ? 'seller' : 'customer';
        const res = await fetch(`/api/orders?userId=${user.id}&userType=${userType}`);
        if (!res.ok) throw new Error('Failed to fetch orders');
        const json = await res.json();
        const data = json.orders ?? [];
        setOrders(data.map((order: any) => ({
          ...order,
          gigTitle: order.gigTitle || order.gig?.title || 'Unknown Gig',
          gigImage: order.gigImage || order.gig?.images?.[0] || null,
          sellerName: order.sellerName || (order.seller?.user ? `${order.seller.user.first_name} ${order.seller.user.last_name}` : 'Unknown Seller'),
          customerName: order.customerName || (order.customer?.user ? `${order.customer.user.first_name} ${order.customer.user.last_name}` : 'Unknown Customer'),
      ...
      <div className="mb-8">
        ...
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-green bg-white shadow-sm"
          />
        </div>
      </div>
